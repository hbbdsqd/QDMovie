...
//            self.titleLabelCenterY.constant = self.titleLabel.bounds.size.height/2 + myCellModel.titleLabelVerticalOffset;
            self.titleLabel.frame = CGRectMake(self.titleLabel.left, 3 + 30 - self.titleLabel.height, self.titleLabel.width, self.titleLabel.height);
            break;
        default:
            break;
    }
    
    NSLog(@"%@",NSStringFromCGRect(self.titleMaskLayer.frame));
    NSLog(@"%@",NSStringFromCGRect(self.maskTitleMaskLayer.frame));
}
...
- (void)reloadData:(JXCategoryBaseCellModel *)cellModel {
    ...
    NSLog(@"%f------%@",self.titleLabelCenterY.constant,NSStringFromCGRect(self.titleLabel.frame));
    switch (myCellModel.titleLabelAnchorPointStyle) {
        case JXCategoryTitleLabelAnchorPointStyleCenter:
